...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        private void Awake()
        {
            // Local camera only matters for the owner
            if (IsOwner)
            {
                if (_mainCamera == null)
        ...
        private void GroundedCheck()
        {
            ...
            if (_hasAnimator)
                _animator.Animator.SetBool(_animIDGrounded, Grounded);
        }
        ...
        private void Move()
        {
            ...
            // normalised input direction (x = right, y = forward on gamepad/WASD)
            Vector3 inputDirection = new Vector3(_input.move.x, 0.0f, _input.move.y).normalized;

            // >>> FIX: rotate character towards input relative to camera
            if (inputDirection != Vector3.zero && _mainCamera != null)
            {
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                if (_hasAnimator)
                {
                    _animator.Animator.SetBool(_animIDJump, false);
                    _animator.Animator.SetBool(_animIDFreeFall, false);
                }
                ...
                if (_input.jump && _jumpTimeoutDelta <= 0.0f)
                {
                    ...
                    if (_hasAnimator)
                        _animator.Animator.SetBool(_animIDJump, true);
                }
            ...
            else
            {
                ...
                {
                    if (_hasAnimator)
                        _animator.Animator.SetBool(_animIDFreeFall, true);
                }
